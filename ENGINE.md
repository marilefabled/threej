...
|---|---|---|
| `npm run dev` | HMR dev server on :5173 | Web (dev) |
| `npm run build` | `dist/` (chunk-split, PWA SW) | Web / PWA |
| `npm run preview` | serve `dist/` locally | Web verification |
| `npm run build:lib` | `dist/engine/*.js` (tree-shakable) | Engine library |
| `npm run dev:electron` | Vite + Electron window | Desktop (dev) |
| `npm run build:electron` | `dist-electron/` .dmg/.exe/.AppImage | Desktop (prod) |
| `npm run build:mobile` | Vite build + `cap sync` → native projects | Mobile |
| `npm run open:ios` | Open Xcode workspace | iOS (needs Xcode) |
| `npm run open:android` | Open Android Studio | Android (needs AS) |
| `npm run typecheck` | `tsc --noEmit` | CI gate |

**Platform targets:**
- **Web** — static hosting (Netlify, Vercel, GitHub Pages, any CDN). `dist/` is
  self-contained. `dist/sw.js` + `dist/manifest.webmanifest` add PWA install support.
- **PWA** — generated by `vite-plugin-pwa`. On Chrome/Edge desktop users see an
  "Install" prompt. On iOS/Android: "Add to Home Screen". Offline caching via Workbox
  (CacheFirst for hashed chunks; 90-day cache for the Rapier chunk). Add real icons:
  drop `pwa-192.png` and `pwa-512.png` (192×192 and 512×512) in `public/`.
- **Desktop (Electron)** — `electron/main.cjs` loads `dist/index.html`. `electron-builder`
  packages to native installers via `npm run build:electron`:
    - macOS → `.dmg` + `.zip` (x64 + arm64 universal)
    - Windows → `.exe` installer + portable
    - Linux → `.AppImage` + `.deb`
  First run downloads Electron binaries (~120 MB, cached in `~/.cache/electron`).
- **Mobile (Capacitor)** — `npm run build:mobile` syncs `dist/` into native wrappers.
  Add platforms once: `npx cap add ios` (macOS + Xcode required) / `npx cap add android`
  (Android Studio required). Then `npm run open:ios` / `open:android` to build + deploy.
  WKWebView (iOS 15+) and ChromiumWebView (Android) both support WebGL 2.0 + WASM.
  `ios/` and `android/` are gitignored (generated; regenerate with `cap add`).
